...
import { PDFName } from '@makepdfform/pdf-lib';
import type { PDFDocument, PDFPage } from '@makepdfform/pdf-lib';
import {
  Schema,
  Template,
  Font,
...
export const getFallbackFontName = (font: Font) => {
  ...
  if (fallbackFontName === initial) {
    throw Error(
      `[@makepdfform/common] fallback flag is not found in font. true fallback flag must be only one.`,
    );
  }
  ...
  return new Promise<string>((resolve, reject) => {
    ...
    reader.onloadend = () => {
      if ((reader.result as string).startsWith('data:application/pdf;')) {
        ...
        reject(Error('[@makepdfform/common] template.basePdf must be pdf data.'));
      }
    };
...
export const applyInternalLinkAnnotations = (arg: {
  ...
  cache.annotations.forEach((annotation) => {
    ...
    if (anchors.length === 0) {
      throw new Error(
        `[@makepdfform/generator] Internal link target "#${annotation.targetName}" was not found.`,
      );
    }
    if (anchors.length > 1) {
      throw new Error(
        `[@makepdfform/generator] Internal link target "#${annotation.targetName}" is ambiguous because multiple schemas use that name.`,
      );
    }
...
export const getB64BasePdf = async (
  ...
  if (
    ...
    if (!isUrlSafeToFetch(customPdf)) {
      throw Error(
        '[@makepdfform/common] Invalid or unsafe URL for basePdf. Only http: and https: URLs pointing to public hosts are allowed.',
      );
    }
...
Check this document: https://pdfme.com/docs/custom-fonts#about-font-type`,
  ...
  if (fallbackFontNum > 1) {
    throw Error(
      `[@makepdfform/common] ${fallbackFontNum} fallback flags found in font. true fallback flag must be only one.
Check this document: https://pdfme.com/docs/custom-fonts#about-font-type`,
    );
  }
  ...
  if (fontNamesInSchemas.some((f) => !fontNames.includes(f))) {
    throw Error(
      `[@makepdfform/common] ${fontNamesInSchemas
...
export const checkPlugins = (arg: { plugins: Plugins; template: Template }) => {
  ...
  if (allSchemaTypes.some((s) => !pluginsSchemaTypes.includes(s))) {
    throw Error(
      `[@makepdfform/common] ${allSchemaTypes
...
--------------------------`,
      ...
      throw Error(`[@makepdfform/common] Invalid argument:
--------------------------
${messages.join('\n')}`);
    } else {
      throw Error(
        `[@makepdfform/common] Unexpected parsing error: ${e instanceof Error ? e.message : String(e)}`,
      );
    }
  }
