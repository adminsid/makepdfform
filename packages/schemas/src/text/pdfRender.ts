import { PDFFont, PDFDocument } from '@makepdfform/pdf-lib';
...
const embedAndGetFont = (arg: {
  ...
  if (!fontValue) {
    return Promise.reject(new Error(`[@makepdfform/schemas] Font "${fontName}" is not found.`));
  }
...
export const pdfRender = async (arg: PDFRenderProps<TextSchema>) => {
  ...
  if (!pdfFontValuePromise) {
    throw new Error('[@makepdfform/schemas] Failed to prepare PDF font for text rendering.');
  }
