import * as pdfLib from '@makepdfform/pdf-lib';
import type { GenerateProps, GeneratorOptions, Schema, PDFRenderProps, Template } from '@makepdfform/common';
import {
  checkGenerateProps,
...
import { getDynamicLayoutForSchema, isDynamicLayoutSchema } from '@makepdfform/schemas/dynamicLayout';
import {
  insertPage,
  preprocessing,
  postProcessing,
...
const generate = async (props: GenerateProps): Promise<Uint8Array<ArrayBuffer>> => {
  ...
  if (inputs.length === 0) {
    throw new Error(
      '[@makepdfform/generator] inputs should not be empty, pass at least an empty object in the array',
    );
  }
