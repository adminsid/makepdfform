...
export const substituteVariables = (
  ...
  if (variablesIn) {
    ...
    try {
      ...
      throw new SyntaxError(`[@makepdfform/schemas] MVT: invalid JSON string '${variablesIn as string}'`);
    }
...
export const validateVariables = (value: string, schema: MultiVariableTextSchema): boolean => {
  ...
  try {
    ...
    throw new SyntaxError(
      `[@makepdfform/generator] invalid JSON string '${value}' for variables in field ${schema.name}`,
    );
  }
  ...
  for (const variable of schema.variables) {
    if (
      ...
      if (schema.required) {
        throw new Error(
          `[@makepdfform/generator] variable ${variable} is missing for field ${schema.name}`,
        );
      }
