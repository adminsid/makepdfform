import { getDefaultFont, isBlankPdf, pt2mm, resolvePageSize } from '@makepdfform/common';
import type { Font, Schema, Template } from '@makepdfform/common';
import type {
  ...
  TableSchema,
  TextSchema,
} from '@makepdfform/schemas/types';
import {
  escapeInlineMarkdown,
  getVariableNames,
  measureTextHeight,
  visitVariables,
} from '@makepdfform/schemas/utils';
...
export const renderToTemplate = async (
  ...
  if (pages.length === 0) {
    throw new Error('@makepdfform/jsx: renderToTemplate root must contain at least one <Page>.');
  }
  ...
  if (hasStaticChildren && options.basePdf != null && !isBlankPdf(options.basePdf)) {
    throw new Error(
      '@makepdfform/jsx: <Header>, <Footer>, and <Static> are supported only with a blank basePdf.',
    );
  }
...
const resolveRenderRoot = (node: PdfJsxChild): RenderRoot => {
  ...
  if (documents.length > 1) {
    throw new Error('@makepdfform/jsx: only one <Document> root is supported.');
  }

  if (topLevelChildren.some((child) => child !== documents[0])) {
    throw new Error('@makepdfform/jsx: <Document> must be the only root element.');
  }
...
const getStaticPlacement = (element: PdfJsxElement<'static'>): StaticPlacement => {
  ...
  if (placement !== 'top' && placement !== 'bottom') {
    throw new Error('@makepdfform/jsx: <Static> placement must be "top" or "bottom".');
  }
  return placement;
};
...
const renderElement = async (
  ...
  switch (element.kind) {
    ...
    case 'document':
      throw new Error('@makepdfform/jsx: <Document> must be the root element.');
    case 'header':
    case 'footer':
    case 'static':
      throw new Error(
        '@makepdfform/jsx: <Header>, <Footer>, and <Static> can only be used as direct children of <Document>.',
      );
    case 'absolute':
...
const renderText = async (
  ...
  if (!readOnly && textFormat === 'inline-markdown') {
    throw new Error(
      '@makepdfform/jsx: editable <Text> does not support textFormat="inline-markdown". Use read-only <Text> or <MultiVariableText>.',
    );
  }
...
const resolveName = (ctx: RenderCtx, prefix: string, userName?: string): string => {
  if (userName) {
    if (ctx.usedNames.has(userName)) {
      throw new Error(`@makepdfform/jsx: duplicate schema name "${userName}"`);
    }
...
const validateConsistentPageProps = (
  ...
  for (let i = 1; i < pages.length; i += 1) {
    ...
    if (!isSameSize(pageSize, firstPageSize) || !isSameBoxSides(margin, firstMargin)) {
      throw new Error(
        '@makepdfform/jsx: all <Page> nodes must use the same size, orientation, and margin. pdfme templates have one blank basePdf size and padding.',
      );
    }
  }
};
  ...
  for (const child of flattenForSplitting(node)) {
    ...
    if (child.kind === 'pagebreak') {
      if (!canBreak || !parentKind || !PAGE_BREAK_PARENT_KINDS.has(parentKind)) {
        throw new Error(
          '@makepdfform/jsx: <PageBreak> can only be used inside <Page>, <Stack>, or <Box>.',
        );
      }
      continue;
    }
...
const validateAbsolutePlacement = (
  ...
  for (const child of flattenForSplitting(node)) {
    ...
    if (child.kind === 'absolute' && (!parentKind || !ABSOLUTE_PARENT_KINDS.has(parentKind))) {
      throw new Error(
        '@makepdfform/jsx: <Absolute> can only be used inside <Page>, <Header>, <Footer>, <Static>, or <Box>.',
      );
    }
  ...
  for (const child of flattenChildren(children)) {
    if (!isPdfJsxElement(child)) {
      if (hasDocument && String(child).trim() !== '') {
        throw new Error(
          '@makepdfform/jsx: <Document> children must be <Header>, <Footer>, <Static>, or <Page>.',
        );
      }
      continue;
    }
    ...
    if (STATIC_BLOCK_KINDS.has(child.kind)) {
      ...
      if (child.kind === 'static') {
        ...
        if (placement === 'bottom' && hasElementKind(child.children, 'absolute')) {
          throw new Error(
            '@makepdfform/jsx: <Absolute> is not supported inside bottom <Static>. Use top <Static>, <Header>, <Footer>, or <Page> for fixed page coordinates.',
          );
        }
      }
...
const validateStaticChildren = (children: PdfJsxChild | PdfJsxChild[]) => {
  for (const child of flattenForSplitting(children)) {
    ...
    if (!STATIC_LEAF_KINDS.has(child.kind)) {
      throw new Error(
        `@makepdfform/jsx: <Static> does not support <${child.kind}> children. Supported: read-only Stack, Row, Box, Spacer, Text, Image, Svg, Rectangle, Ellipse, and Line.`,
      );
    }
...
const validateStaticLeafProps = (element: PdfJsxElement) => {
  ...
  if (props.readOnly === false || (props.name != null && props.readOnly !== true)) {
    throw new Error('@makepdfform/jsx: <Static> children must be read-only.');
  }
};
