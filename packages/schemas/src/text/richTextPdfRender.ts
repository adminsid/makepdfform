import type { PDFFont, Rotation } from '@makepdfform/pdf-lib';
import type { ColorType, Font, PDFRenderProps } from '@makepdfform/common';
import { getInternalLinkTarget, registerInternalLinkAnnotation } from '@makepdfform/common';
...
const getPdfFontFromObj = (run: RichTextLineRun, pdfFontObj: Record<string, PDFFont>) => {
  ...
  if (!pdfFont) {
    throw new Error(`[@makepdfform/schemas] Missing embedded font "${run.fontName}".`);
  }
  return pdfFont;
};
