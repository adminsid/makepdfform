...
export const fetchRemoteFontData = async (url: string): Promise<ArrayBuffer> => {
  if (!isUrlSafeToFetch(url)) {
    throw Error(
      '[@makepdfform/schemas] Invalid or unsafe URL for font data. Only http: and https: URLs pointing to public hosts are allowed.',
    );
  }
  ...
  try {
    ...
    throw Error(`[@makepdfform/schemas] Failed to fetch remote font data from ${url}. ${reason}`);
  }
};
