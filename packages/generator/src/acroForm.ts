...
import { convertForPdfLayoutProps, hex2PrintingColor } from '@makepdfform/schemas/utils';
...
const fetchFontData = async (font: Font, fontName: string) => {
  ...
  if (!fontValue) {
    throw new Error(`[@makepdfform/generator] Font "${fontName}" is not configured`);
  }
  ...
  if (!res.ok) {
    throw new Error(`[@makepdfform/generator] Failed to fetch font data from ${fontValue.data}`);
  }
