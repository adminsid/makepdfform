...
import {
  ...
  pluginRegistry,
  BasePdf,
} from '@makepdfform/common';
import { builtInPlugins } from '@makepdfform/schemas/builtins';
import {
  PDFPage,
  PDFDocument,
  PDFEmbeddedPage,
...
const loadBasePdfWithPassword = async (
  ...
  try {
    ...
    throw new Error(
      '[@makepdfform/generator] basePdf is encrypted and requires a valid password. Pass options.basePdfPassword to generate().',
    );
  }
};
  ...
  template.schemas.forEach((schemaPage: Schema[]) =>
    schemaPage.forEach((schema: Schema) => {
      if (schema.required && !schema.readOnly && !inputs.some((input) => input[schema.name])) {
        throw new Error(
          `[@makepdfform/generator] input for '${schema.name}' is required to generate this PDF`,
        );
      }
    }),
  );
  ...
  const renderObj = schemaTypes.reduce(
    (
      ...
      if (!plugin || !plugin.pdf) {
        throw new Error(`[@makepdfform/generator] Plugin or renderer for type ${type} not found.
