...
import { cmyk, degrees, degreesToRadians, rgb, Color } from '@makepdfform/pdf-lib';
import { Schema, mm2pt, Mode, isHexValid, ColorType } from '@makepdfform/common';
...
export const readFile = (input: File | FileList | null): Promise<string | ArrayBuffer> =>
  new Promise((resolve, reject) => {
    ...
    fileReader.onerror = () => {
      reject(new Error('[@makepdfform/schemas] File reading failed'));
    };
    ...
    if (file) {
      ...
      reject(new Error('[@makepdfform/schemas] No files provided'));
    }
  });
